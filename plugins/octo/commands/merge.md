...
2. Show its status: checks (`gh pr checks`), review state, and mergeable/conflicts
   (`gh pr view`).
3. If checks are failing or it isn't approved, warn the developer and confirm before going on.
4. Merge (`gh pr merge`) — ask the method (default **squash**), and offer to delete the
   branch afterwards.
5. Report the result and the merge commit.
