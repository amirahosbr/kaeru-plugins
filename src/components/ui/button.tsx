/**
 * Button — owned shadcn primitive.
 *
 * Variants are colocated here via `cva` (no central styles file). All colors
 * reference semantic tokens so the button survives `.dark` and re-theming.
 * Renders as a `<button>`, or via `asChild` merges props onto its child
 * (e.g. wrap a `<Link>`) using Radix `Slot`.
 */
